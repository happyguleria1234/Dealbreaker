...
//        layer.addSublayer(maxLabel)
    ...
    private func updateLabelValues() {
        ...
        if let nsstring = minLabel.string as? NSString {
            minLabelTextSize = nsstring.size(withAttributes: [kCTFontAttributeName as NSAttributedString.Key: minLabelFont])
        }

        if let nsstring = maxLabel.string as? NSString {
            maxLabelTextSize = nsstring.size(withAttributes: [kCTFontAttributeName as NSAttributedString.Key: maxLabelFont])
        }
    }
